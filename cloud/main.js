...
Parse.Cloud.define("sendEmailWelcomeNewUser", function(request, response) {
	...
	mailgun.messages().send({
	  ...
	  bcc: CFA_NEMP_EMAIL,
	  from: CFA_NEMP_EMAIL,
	  subject: "Welcome to the South Australia " + process.env.APP_NAME,
	  text: "",
	  html: html
    }, function (error, body) {
      if (error)
...
Parse.Cloud.define("sendEmailFinalisedDataToUsers", function(request, response) {
	...
	queryMMR.find({ useMasterKey: true }).then(function(results) {
		...
		mailgun.messages().send({
			...
			bcc: recipientList,
			from: CFA_NEMP_EMAIL,
			subject: "South Australia Grassland Curing Map - " + strToday,
			text: "",
			html: html
		}, function (error, body) {
			if (error)
...
Parse.Cloud.beforeSave("GCUR_OBSERVATION", async (request) => {
	...
	let newAreaCuring = newValidatorCuring = newAdminCuring = undefined;
	...
	console.log("*** beforeSave: AreaCuring[ " + newAreaCuring + "], ValidatorCuring[" + newValidatorCuring + "], AdminCuring[" + newAdminCuring + "]");
	...
	// Updating an existing GCUR_OBSERVATION object
		...
		if ( (newAreaCuring == undefined) && (newValidatorCuring == undefined) && (newAdminCuring == undefined) ) {
			console.log("newAreaCuring and newValidatorCuring and newAdminCuring are all undefined. Delete this observations.");
